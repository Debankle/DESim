...
class SimulationQueue:
    def __init__(self):
        self.sqs = get_service("sqs", SQSService)
        self.s3 = get_service("s3", S3Service)
        self.total_units = psutil.virtual_memory().total / (1024**2) * 0.85
    ...
    def dispatcher(self):
        threshold = self.resource_mgr.total * 0.15

        while not self.stop:
            with self.resource_mgr.cond:
                while self.resource_mgr.available < threshold:
                    self.resource_mgr.cond.wait()

            try:
                receipt, job = self.sqs.receive_job()
            except ClientError:
                continue
    ...
    def _heartbeat(self, receipt, hb_stop):
        while not hb_stop.wait(30):
            try:
                self.sqs.extend_visibility(receipt)
            except ClientError:
                break
