...
def update_simulation_status(sim_id, status, message=None):
    db = get_service("db", DBService)
    try:
        if not message:
            db.execute(
            ...
            db.execute(
                "update simulations set status=%s, message=%s where simulation_id=%s",
                (status, message, sim_id),
            )
    except psycopg.Error as e:
        _map_psycopg_error(e)


def complete_simulation(sim_id):
    db = get_service("db", DBService)
    try:
        db.execute(
            "update simulations set status=%s, complete_time=%s where simulation_id=%s",
            ("complete", datetime.datetime.now(), sim_id),
        )
    except psycopg.Error as e:
        _map_psycopg_error(e)
